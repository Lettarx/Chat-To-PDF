<script>
    ...
    const numOfImageToShow = Math.min(pages,4)
    const images = Array.from({length: numOfImageToShow}, (_, i) => {
        const page = i + 1
        return url
...
{#if loading}
    <div class="flex justify-center items-center mt-10 flex-col gap-y-2">
        <Spinner/>
